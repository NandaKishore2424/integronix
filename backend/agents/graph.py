...
class CodingState(TypedDict, total=False):
    ...
    procedures_and_services: List[str] # Extracted by Node 2 LLM
    ...
    # --- Financial Summary ---
    financial_summary: Optional[dict]  # Output from financial_calculator node
...
def _route_after_mapping(state: CodingState) -> str:
    """Skip embedding whenever any upstream node (WHO API, direct SNOMED
    crosswalk, or any fallback) has already produced candidates."""
    has_candidates: bool = bool(state.get("candidate_icd_codes"))

    if has_candidates:
        return "icd_decision"   # WHO API / direct path → skip embedding
    return "icd_embedding"      # No candidates yet → run embedding fallback
...
def build_integronix_graph():
    ...
    # Ensure `langgraph` is installed or replace with an alternative
    try:
