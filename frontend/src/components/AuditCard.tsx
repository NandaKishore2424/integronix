        ...
        <div className="glass-card p-6 flex flex-col gap-5">
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <div className="rounded-xl border border-accent/20 bg-accent/[0.06] p-4">
                    <p className="mono-label mb-2">Recommended Code</p>
                    ...
                    {d.ai_is_mcc && <span className="mt-2 inline-block text-[10px] font-bold px-2 py-0.5 rounded bg-danger/15 border border-danger/30 text-danger-light">High Severity</span>}
                    {d.ai_is_cc && !d.ai_is_mcc && <span className="mt-2 inline-block text-[10px] font-bold px-2 py-0.5 rounded bg-warning/15 border border-warning/30 text-warning">Complication</span>}
                </div>

                {/* Submitted */}
                <div className="rounded-xl border border-white/[0.06] bg-white/[0.03] p-4">
                    <p className="mono-label mb-2">Submitted Code</p>
            ...
            <div className="rounded-xl bg-white/[0.03] border border-white/[0.06] p-4">
                <p className="text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">Clinical Rationale</p>
            ...
            {delta !== 0 && (
                ...
                >
                    <div>
                        <p className="mono-label mb-1">Financial Impact</p>
                        <p className="text-3xl font-extrabold" style={{ color: delta > 0 ? '#4ade80' : '#f87171' }}>
                            {formatCurrency(delta)}
                        </p>
                        <p className="text-xs text-slate-500 mt-0.5">
                            {delta > 0 ? 'Estimated revenue recovery per claim' : 'Compliance exposure per claim'}
                        </p>
                    </div>
                    {delta > 0
